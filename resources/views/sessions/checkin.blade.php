...
<div class="max-w-2xl mx-auto px-4 py-12">
    ...
    <form method="POST" action="{{ route('sessions.checkin.store', $session) }}" class="flex flex-col sm:flex-row gap-2 mb-8">
        ...
        <input type="tel" name="phone" placeholder="Phone"
            class="sm:w-36 bg-gray-50 border-none rounded-2xl px-5 py-4 font-bold outline-none focus:ring-2 focus:ring-[#F07F22]/20">
        <input type="email" name="email" placeholder="Email"
            class="sm:w-44 bg-gray-50 border-none rounded-2xl px-5 py-4 font-bold outline-none focus:ring-2 focus:ring-[#F07F22]/20">
            ...
            <div x-data="{ editing: false }" class="bg-white rounded-2xl border border-gray-100">
                <div x-show="!editing" class="flex items-center justify-between px-5 py-3">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <a href="{{ route('sessions.checkin.card', [$session, $p]) }}" target="_blank"
   class="text-[9px] font-black uppercase tracking-widest text-[#1D4069] hover:text-[#F07F22]">Card</a>
                    </div>
                </div>
                ...
                <form x-show="editing" method="POST" action="{{ route('sessions.checkin.update', [$session, $p]) }}" class="px-5 py-4 space-y-2">
                    ...
                    <span class="text-[9px] font-black uppercase tracking-widest px-3 py-1.5 rounded-full bg-emerald-50 text-emerald-600">Checked In</span>
                    @if($p->notified_at)
                        <span class="text-[9px] font-black uppercase tracking-widest px-3 py-1.5 rounded-full bg-blue-50 text-blue-500">Notified ✓</span>
                    @endif
